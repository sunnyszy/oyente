...
def check_earlypay_bug(analysis):
    solver = Solver()
    solver.push()
    x = Int('x')
    precondition = [x <= IntVal(1)]
    curcondition = [x == IntVal(len(analysis['money_flow']))]
    solver.add(precondition)
    solver.add(curcondition)
    ret = False
...
def update_analysis(analysis, opcode, stack, mem, global_state, path_conditions_and_vars, cur):
    ...
    if opcode == "SLOAD":
        ...
        if len(stack) > 1:
            ...
            has_earlypay_bug = check_earlypay_bug(analysis)
            # print analysis["money_flow"]
            if (has_earlypay_bug):
                # print """early pay bugs found in line %d""" %cur
                analysis["earlypay_bug"].append(cur)
        # print("money length")
        # print(len(analysis["money_flow"]))
        else:
            raise ValueError('STACK underflow')
